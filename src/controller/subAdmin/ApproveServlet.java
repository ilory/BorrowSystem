...
public class ApproveServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Person_Equipment> list = mapper.readValue(req.getParameter("dataform"),List.class);
        for(int i=0;i<list.size();i++){
            System.out.println(mapper.writeValueAsString(list.get(i)));
            Person_Equipment personEquipment = mapper.readValue(mapper.writeValueAsString(list.get(i)),Person_Equipment.class);
            System.out.println(personEquipment);
            if(!subAdminService.equipmentBorrow(personEquipment)){
                status="-1";
                break;
            }
            status="0";
